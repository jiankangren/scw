...
class ModelTest(Callback):
	...
	def __init__(self, Xt, Yt, T=10, test_every_X_epochs=1, batch_size=500, verbose=1,
				 loss=None, mean_y_train=None, std_y_train=None, arglist=None):
	...
	def predict_stochastic(self, X, batch_size=128, verbose=0):
		...
		return training_arrays.predict_loop(self.model, self._predict_stochastic, X, batch_size, verbose)

	def on_epoch_end(self, batch, logs=None):
		model_json = self.model.to_json()
		with open(self.arglist.save_dir + self.arglist.exp_name + '_model.json', "w") as json_file:
			json_file.write(model_json)
		# serialize weights to HDF5
		self.model.save_weights(self.arglist.save_dir + self.arglist.exp_name + '_model.h5')
		print("Saved model to disk")
	def ons_epoch_begin(self, epoch, logs={}):
		...
		if self.loss == 'binary':
			...
			#with open(self.arglist.save_dir + self.arglist.exp_name + '_data.json', "w") as json_file:
			#	json.dump(MC_err,json_file)
